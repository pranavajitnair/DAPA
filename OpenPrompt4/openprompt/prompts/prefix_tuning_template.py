...
class PrefixTuningTemplate(Template):
    ...
    def __init__(self,
        ...
        self.generate_parameters(file) # in prefix tuning the template text has no interact with the parameters.
    ...
    def on_text_set(self):
        ...
        self.generate_parameters(self.file)
    ...
    def get_past_key_values(self, batch_size=1):
        ...
        if self.config.is_encoder_decoder and self.using_encoder_past_key_values:
            ...
            self.wte=nn.Embedding.from_pretrained(torch.load(self.file))
        ...
        if (not self.config.is_encoder_decoder) or self.using_decoder_past_key_values:
            ...
            self.decoder_wte=nn.Embedding.from_pretrained(torch.load(self.file))
        ...
        if  (not self.config.is_encoder_decoder) or self.using_cross:
            self.cross_wte=nn.Embedding.from_pretrained(torch.load(self.file))
    ...
    def generate_parameters(self,file) -> None:
        r"""
        ...
        if self.config.is_encoder_decoder and self.using_encoder_past_key_values:
            self.wte=nn.Embedding.from_pretrained(torch.load(file))
        ...
        if (not self.config.is_encoder_decoder) or self.using_decoder_past_key_values:
            self.decoder_wte=nn.Embedding.from_pretrained(torch.load(file))
        ...
        if  (not self.config.is_encoder_decoder) or self.using_cross:
            self.cross_wte=nn.Embedding.from_pretrained(torch.load(file))
