...
class PrefixTuningTemplate(Template):
    ...
    def get_past_key_values(self, prefix, batch_size=1):
        pvs = []
        ...
        if (not prefix.config.is_encoder_decoder) or prefix.using_decoder_past_key_values:
            ...
            decoder_temp_control = prefix.decoder_wte(decoder_input_tokens)
        ...
        if  (not prefix.config.is_encoder_decoder) or prefix.using_cross:
            ...
            cross_temp_control = prefix.cross_wte(cross_input_tokens)
    ...
    def get_past_key_values1(self, batch_size = 1):
        prefix11 = self.get_past_key_values(self.prefix1)
        prefix22 = self.get_past_key_values(self.prefix2)
        prefix33 = self.get_past_key_values(self.prefix3)
        ...
        enc = torch.cat([enc1, enc2, enc3], dim = -1)
        dec = torch.cat([dec1, dec2, dec3], dim = -1)
        cross = torch.cat([cross1, cross2, cross3], dim = -1)

        enc_q = self.q(enc)
        enc_k = self.k(enc)
        enc_v = self.v(enc)
        enc_scores = torch.nn.functional.softmax(torch.matmul(enc_q, enc_k.transpose(-1,-2))/(self.n_embd**0.5),dim=-1)
        enc_scores = torch.matmul(enc_scores,enc_v)
        pvs.append(enc_scores.split(2))

        dec_q = self.q(dec)
        dec_k = self.k(dec)
        dec_v = self.v(dec)
        dec_scores = torch.nn.functional.softmax(torch.matmul(dec_q, dec_k.transpose(-1,-2))/(self.n_embd**0.5),dim=-1)
        dec_scores = torch.matmul(dec_scores,dec_v)
        pvs.append(dec_scores.split(2))

        cross_q = self.q(cross)
        cross_k = self.k(cross)
        cross_v = self.v(cross)
        cross_scores = torch.nn.functional.softmax(torch.matmul(cross_q, cross_k.transpose(-1,-2))/(self.n_embd**0.5),dim=-1)
        cross_scores = torch.matmul(cross_scores,cross_v)
        pvs.append(cross_scores.split(2))
    ...
    def generate_parameters(self) -> None:
        ...
        if self.config.is_encoder_decoder and self.using_encoder_past_key_values:
            self.k = nn.Linear(self.match_n_embd*3,self.match_n_embd)
            self.q = nn.Linear(self.match_n_embd*3,self.match_n_embd)
            self.v = nn.Linear(self.match_n_embd*3,self.match_n_embd)
    
        if (not self.config.is_encoder_decoder) or self.using_decoder_past_key_values:
            self.decoder_k = nn.Linear(self.match_n_embd*3,self.match_n_embd)
            self.decoder_q = nn.Linear(self.match_n_embd*3,self.match_n_embd)
            self.decoder_v = nn.Linear(self.match_n_embd*3,self.match_n_embd)

        if  (not self.config.is_encoder_decoder) or self.using_cross:
            self.encoder_k = nn.Linear(self.match_n_embd*3,self.match_n_embd)
            self.encoder_q = nn.Linear(self.match_n_embd*3,self.match_n_embd)
            self.encoder_v = nn.Linear(self.match_n_embd*3,self.match_n_embd)
    ...
    def process_batch(self, batch: Union[Dict, InputFeatures]) -> Union[Dict, InputFeatures]:
        ...
        self.past_key_values = self.get_past_key_values1()
