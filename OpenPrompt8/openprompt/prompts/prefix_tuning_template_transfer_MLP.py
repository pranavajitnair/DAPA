...
class PrefixTuningTemplate(Template):
    ...
    def get_past_key_values1(self, batch_size = 1):
        ...
        if self.config.is_encoder_decoder and self.using_encoder_past_key_values:
            past_key_values = self.control_trans(enc) #bsz, seqlen, layer*emb
            past_key_values = self.dropout(past_key_values)
            pvs.append(past_key_values.split(2))
        else:
            pvs.append(None)

        if (not self.config.is_encoder_decoder) or self.using_decoder_past_key_values:

            decoder_past_key_values = self.decoder_control_trans(dec) #bsz, seqlen, layer*emb
            decoder_past_key_values = self.dropout(decoder_past_key_values)
            pvs.append(decoder_past_key_values.split(2))
        ...
        if  (not self.config.is_encoder_decoder) or self.using_cross:
            cross_past_key_values = self.cross_control_trans(cross) #bsz, seqlen, layer*emb
            cross_past_key_values = self.dropout(cross_past_key_values)
            pvs.append(cross_past_key_values.split(2))
    ...
    def generate_parameters(self) -> None:
        ...
        if self.config.is_encoder_decoder and self.using_encoder_past_key_values:
            self.control_trans = nn.Sequential(
                nn.Linear(self.match_n_embd*3, self.mid_dim),
                nn.Tanh(),
                # nn.Linear(self.mid_dim, self.mid_dim),
                # nn.Tanh(),
                nn.Linear(self.mid_dim, self.match_n_embd))
